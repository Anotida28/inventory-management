...
export class ReportsService {
  ...
  async getDashboard(filters: ReportFiltersDto, mode: SystemMode) {
    ...
    transactions.forEach((txn) => {
      ...
      if (txn.type === "RECEIVE") {
        ...
        bucket.cost += cost;
        if (!isInventory) bucket.profit -= cost;
      }
  ...
  async getUserActivity(filters: ReportFiltersDto) {
    ...
    transactions.forEach((txn) => {
      ...
      entry.transactions.push(toTransactionShape(txn));
      entry.counts[txn.type] = (entry.counts[txn.type] || 0) + 1;

      if (txn.type === "RECEIVE") entry.receipts += 1;
      if (txn.type === "ISSUE") entry.issues += 1;
      if (txn.type === "ADJUSTMENT") entry.adjustments += 1;
      if (txn.type === "REVERSAL") entry.reversals += 1;
    });
